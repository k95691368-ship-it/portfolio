...
export function parseCookie(request, name) {
  // Supabase Edge Functions run on a different origin from the static site, so
  // HttpOnly same-site cookies cannot be the transport there. Keep cookie
  // support for local/legacy requests, while accepting the same opaque session
  // tokens from narrowly scoped headers. X-App-Authorization is kept separate
  // from Supabase's gateway Authorization header.
  const authorizationHeader =
    name === 'session'
      ? request.headers.get('X-App-Authorization') || request.headers.get('Authorization')
      : name === ROOM_COOKIE
        ? request.headers.get('X-Room-Authorization')
        : null
