...
function providerErrorStatus(error) {
  return error instanceof VideoServiceConfigError ? 503 : 502
}

function alreadyAbsent(error) {
  return error instanceof VideoServiceError && error.status === 404
}
...
async function revokeMembership(env, member) {
  ...
  try {
    ...
    if (!isMeetingAlreadyEnded(error)) {
  ...
  try {
    ...
    await closeMeeting(env, { meetingId: member.provider_meeting_id })
  } catch (error) {
    if (!isMeetingAlreadyEnded(error)) {
