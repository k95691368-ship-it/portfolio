...
export async function onRequestGet({ request, env, data, params }) {
  ...
  const wantsSignedUrl = new URL(request.url).searchParams.get('signed') === '1'
  if (wantsSignedUrl && env.INTERVIEW_RECORDINGS.createSignedUrl) {
    const signed = await env.INTERVIEW_RECORDINGS.createSignedUrl(
      recording.r2_key,
      300,
      wantsDownload ? safeFilename(recording.filename, `${recording.id}.webm`) : false
    )
    await logAccess(env, request, recording.id, data.user.id, action, 'allowed', 'signed_url')
