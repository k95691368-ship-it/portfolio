...
async function stopRecordingAfterRemovalFailure(env, sessionId) {
  ...
  try {
    const result = normalizeRecordingResult(
    ...
    await env.DB.prepare(
      ...
      .bind(result?.status || 'processing', result?.stoppedAt, recording.id)
      .run()
    return true
  } catch (error) {
...
async function revokeAdmission(env, session, member) {
  ...
  if (failures.length) {
    const failure = new Error('화상 참가자 퇴장 또는 연결 해제를 확인하지 못했습니다.')
...
async function failClosedInterview(env, session) {
  ...
  try {
    ...
    activeSessionEnded = isMeetingAlreadyEnded(error)
  ...
  try {
    await closeMeeting(env, { meetingId: session.provider_meeting_id })
    meetingDeactivated = true
  } catch (error) {
    meetingDeactivated = isMeetingAlreadyEnded(error)
