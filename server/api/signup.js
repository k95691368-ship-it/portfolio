import { genId } from '../_lib/db.js'
import { hashPassword, createSession, sessionCookieHeader, normalizeEmail } from '../_lib/auth.js'
import { jsonResponse, jsonError } from '../_lib/http.js'
import { checkRateLimit } from '../_lib/rateLimit.js'
import { isPasswordInput, validateAccountProfile } from '../_lib/accountInput.js'

export async function onRequestPost({ request, env }) {
  const body = await request.json().catch(() => null)
  if (!body) return jsonError('잘못된 요청입니다.', 400)
  const profileError = validateAccountProfile(body)
  if (profileError) return jsonError(profileError, 400)

  const { password, role, displayName, companyName } = body
  // 지원서 쪽과 같은 표기로 통일해 같은 사람이 두 계정으로 갈라지지 않게 한다.
  const email = normalizeEmail(body.email)
  if (!isPasswordInput(password, 8)) {
    return jsonError('비밀번호는 8~1024자의 문자열이어야 합니다.', 400)
  }

  const ip = request.headers.get('CF-Connecting-IP') || 'unknown'
  const allowed = await checkRateLimit(env, `signup:${ip}`, 10, 3600)
  if (!allowed) return jsonError('잠시 후 다시 시도해주세요.', 429)

  const { hash, salt } = await hashPassword(password)
  const id = genId()
  try {
    await env.DB.prepare(
      `INSERT INTO users (id, email, password_hash, password_salt, role, display_name, company_name)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    )
      .bind(id, email, hash, salt, role, displayName.trim(), companyName?.trim() || null)
      .run()
  } catch (err) {
    if (String(err?.message || err).includes('UNIQUE')) {
      return jsonError('이미 가입된 이메일입니다.', 409)
    }
    throw err
  }

  // 회원가입에는 '로그인 유지' 선택 자체가 없어 언제나 30일이었다. 로그인
  // 화면에는 있는 선택이다 — 같은 계정을 만드는 자리에서 한쪽만 강제로 유지되면
  // PC방에서 가입한 사람이 창을 닫아도 세션이 남는다. 로그인 쪽과 맞춘다.
  const persistent = body.remember !== false
  const { token, expiresAt } = await createSession(env.DB, id, { persistent })

  return jsonResponse(
    {
      id,
      email,
      role,
      displayName: displayName.trim(),
      isAdmin: false,
      isRecruiter: false,
      isDeveloper: false,
      mustChangePassword: false,
      sessionToken: token,
      sessionExpiresAt: expiresAt,
      sessionPersistent: persistent,
    },
    201,
    { 'Set-Cookie': sessionCookieHeader(token, { persistent }) }
  )
}
