...
//
// 지원서는 이름·연락처·경력·첨부까지 한 화면에서 다 쓰게 되어 있다. 다 쓰고
// 나서 실수로 뒤로 가거나 탭을 닫으면 전부 사라진다. 로그인 없이 지원하는
// 구조라 임시 저장된 것도 없다. 그래서 "쓰다 만 것이 있는가"를 판단해,
// 떠나기 전에 한 번 되묻는다.
//
// 지원 경로·자기소개는 화면에서 뺐지만 여기서는 계속 본다. 예전에 그 값이
// 실려 오는 경로가 남아 있을 수 있고, 쓴 것이 있는데 없다고 판단하는 쪽이
// 반대보다 나쁘다 — 되묻지 않고 지워 버리게 된다.
