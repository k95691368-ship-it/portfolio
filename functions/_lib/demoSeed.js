...
// 아무도 하지 않는다.
//
// 그래서 이미 그 지점까지 진행된 계약을 미리 심어 둔다. 아이디 하나로 한 시간이면
// 전체를 훑어 보게 하는 것이 목적이다.
//
...
export const DEMO_MESSAGES = {
  ...
  // 이 대화에 채용내정이 성립하는 순간이 들어 있어야 한다.
  //
  // 이 서비스가 존재하는 이유는 회사가 자기가 언제 선을 넘었는지 모른 채
  // 취소하는 것을 막는 데 있다. 그런데 데모 대화에 확정 발화가 하나도 없으면
  // 평가자는 그 기능이 동작하는 것을 볼 수 없다. 담당자가 "아직 협의 중"이라고
  // 생각하면서 실제로는 계약을 세우는 그 말 — 최종 합격과 출근일 — 을 넣는다.
  pending: [
    ['company', '안녕하세요 최민수님. 최종 합격하셨습니다. 9월 1일부터 출근해 주세요.'],
    ['candidate', '감사합니다! 준비해서 나가겠습니다.'],
    ['company', '계약 조건 초안을 계약서에 정리해 두었습니다. 확인 부탁드립니다.'],
...
// 평가자가 밟을 동선. 화면이 이것을 그대로 그린다.
//
// 예전에는 이 목록이 서버에 있는데 아무 데도 나가지 않았고, 화면은 같은 내용을
// 따로 적어 두고 있었다. 방 제목이 세 곳(방 정의·이 목록·화면)에 중복돼 있어서,
// 제목을 고치면 안내만 조용히 옛 이름을 가리킨다. 평가자는 안내가 가리키는
// 방을 찾지 못한다.
//
// 제목은 방 정의에서 가져온다. 한 곳만 고치면 안내도 함께 바뀐다.
export const DEMO_WALKTHROUGH = [
  // 이 서비스가 존재하는 이유부터 보게 한다.
  //
  // 안내가 법령 점검부터 시작하고 있었다. 그것도 이 앱이 하는 일이지만,
  // 만들어진 이유는 회사가 모르고 채용내정을 취소해 손해배상을 무는 것을
  // 막는 것이다. 가장 먼저 보여 줄 것을 세 번째에도 넣지 않으면, 보는 사람은
  // 이것을 "계약서 검사기"로 이해하고 나간다.
  {
    step: 1,
    title: '채용내정이 성립한 것을 알아채고 취소를 막는다',
    body: `"${DEMO_ROOM_PENDING.title}" 방을 열면 대화 위에 경고가 먼저 떠 있습니다. 회사가 보낸 ‘최종 합격하셨습니다. 9월 1일부터 출근해 주세요’ 가 근거로 인용됩니다. 판례는 채용내정 통지로 근로계약이 이미 성립한 것으로 보므로, 여기서 전형을 끝내면 취소가 아니라 해고입니다. 아래 채용내정 취소 칸을 눌러 보세요 — 무엇이 걸리는지, 지원자가 무엇을 할 수 있는지, 취소가 인정될 수 있는 사유가 무엇인지 보여 주고, 확인 표시가 있어야 진행됩니다.`,
  },
  {
    step: 2,
    title: '법령 점검이 실제로 막는 것을 본다',
    body: `"${DEMO_ROOM_PENDING.title}" 방 → 계약서. 최저임금 미달·주 52시간 초과·연차 미기재·기간제 2년 초과·수습 감액 한도·공고보다 불리한 조건이 한 화면에 잡힙니다. 서명 버튼을 눌러도 서버가 막습니다.`,
  },
  {
    step: 3,
    title: '체결된 계약에서 증명서를 발급한다',
    body: `"${DEMO_ROOM_SIGNED.title}" 방 → 계약서 아래 감사추적증명서. 서명·문서 지문·교부와 열람 시각·계약 이력이 담긴 증명서가 발급번호와 함께 나옵니다.`,
  },
  {
    step: 4,
    title: '로그인 없이 진위를 확인한다',
    body: '로그아웃하고 발급번호를 확인 화면에 넣으면 검증됩니다. 정본 문자열을 내려받아 자기 도구로 SHA-256을 계산해 대조할 수도 있습니다.',
  },
]