...
export function checkProbationCompliance(terms) {
  ...
  // 최저임금법 제5조 제2항 단서: "다만, 단순노무업무로 고용노동부장관이 정하여
  // 고시한 직종에 종사하는 근로자는 제외한다."(2018.3.20 신설) 고시는 그 직종을
  // 한국표준직업분류 대분류 9(단순노무 종사자)로 정한다.
  //
  // 업무의 내용은 자유 입력이라 코드가 직종을 분류할 수 없다. 분류할 수 없는
  // 것을 분류한 척하면 틀린 판정을 확신처럼 내미는 셈이다. 대신 감액이 있는
  // 계약에는 이 단서가 있다는 사실을 알려, 사람이 확인하게 한다.
  if (reduced) {
    issues.push({
      severity: 'info',
      title: '단순노무업무면 수습 감액 자체가 금지됩니다',
      detail:
        '단순노무업무(한국표준직업분류 대분류 9 — 청소·경비·배달·단순 제조·운반 등)에 종사하는 근로자는 수습 기간에도 최저임금을 100% 지급해야 합니다(최저임금법 제5조 제2항 단서). 업무의 내용이 이에 해당하는지 확인해주세요.',
    })
  }
