...
export async function onRequestDelete({ env, data, params }) {
  ...
  // 어느 상태로 되돌릴지는 계약 조건이 있는지로 정한다. 조건까지 만들어 둔
  // 방을 'open'으로 되돌리면 진행 상황이 사라진 것처럼 보인다.
  const terms = await env.DB.prepare('SELECT 1 AS found FROM contract_terms WHERE room_id = ?')
    .bind(params.roomId)
    .first()
  const restored = terms ? 'contract_pending' : 'active'
